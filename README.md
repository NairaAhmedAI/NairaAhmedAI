<div align="center" width="50%">

<!-- 🌸 Cute pink-themed girl coding GIF -->
<img src="https://i.pinimg.com/originals/9d/1b/cc/9d1bcc5b4a1b0b3a1d4d8e3d4a47e9f3.gif" alt="Cute girl coding with cat" width="60%"/>  

<!-- 🎧 Animated cozy coding video -->
<video src="https://i.imgur.com/kxv8lqM.mp4" width="60%" autoplay loop muted></video>

<br>
<h1 style="color:#FF69B4;"> Naira Ahmed 👑</h1>
<p style="color:#FFB6C1;"> Deep Learning Engineer | Pythonista | Vision & NLP Enthusiast</p>

<img src="https://media1.tenor.com/m/ITc1hNBSH_wAAAAd/coding-typing.gif" width="690" height="690" alt="a cartoon of a girl looking at a laptop with the words effect credit written above her" fetchpriority="high" style="max-width: 690px;">

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=2000&color=FF69B4&center=true&vCenter=true&width=500&lines=AI+Engineer+%7C+Data+Scientist;Deep+Learning+with+PyTorch;Computer+Vision+%26+NLP+Developer;I'm+Just+Gril+💫)](https://git.io/typing-svg)

</div>

---

<div align="center">

[![LinkedIn Badge](https://img.shields.io/badge/LinkedIn-Naira%20Ahmed-FF69B4?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/naira-ahmed-613171324/)

</div>

---

## 🔥 Tools & Technologies I use

![Python](https://img.shields.io/badge/Python-FFD43B?style=flat&logo=python&logoColor=darkgreen)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![Transformers](https://img.shields.io/badge/HuggingFace_Transformers-FFB6C1?style=flat&logo=huggingface&logoColor=black)
![OpenCV](https://img.shields.io/badge/OpenCV-FF69B4?style=flat&logo=opencv&logoColor=white)
![Solidity](https://img.shields.io/badge/Solidity-FF8DA1?style=flat&logo=solidity&logoColor=white)
![Git](https://img.shields.io/badge/GIT-E44C30?style=flat&logo=git&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-FF69B4?style=flat&logo=visual-studio-code&logoColor=white)


---

```python
class NairaAhmed:
    def __init__(self):
        self.role = "💫 AI Engineer | Deep Learning Specialist"
        self.skills = [
            "🐍 Python", "🔥 PyTorch", "🤗 Transformers", 
            "👁️ OpenCV", "🧠 NLP", "🧩 Model Deployment"
        ]
        self.focus = ["Computer Vision", "Ethical AI", "Innovation"]
    
    def say_hi(self):
        print("Hi there, I'm Naira 👩‍💻 — building the future with AI 💗")

me = NairaAhmed()
me.say_hi()



